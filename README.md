...
XEP-0454 — **131 of 131, nothing skipped**, and then repeats the lane against
...
this suite is green at "2 passed, 129 skipped" on a bare runner and green at "131
...
40, 45, 52, 59, 65, 71, 89, 93, 99, 103, 107, 113, 117, 121, 127, 131 so far, and `nightly.yml` carries the history beside the figure together with
...
| XEP-0084 / XEP-0163 | User Avatar over PEP | ✅ | Three rounds against each service, and what they check is **not ours**: that the server pushes a node to a contact whose caps ask for it. Nothing in the test asks for the change — Alice publishes and Bob is told, and the roster subscription, the caps and the decision to deliver are all the server's bookkeeping. It is also the first lane here that needs the two accounts to know each other; rooms, archives and uploads all work between strangers. Its first run found that **neither far side had personal eventing switched on** — `mod_pep` on Prosody, `mod_pubsub` with the `pep` plugin and `mod_caps` on ejabberd — which is the fourth time in this suite that a module was there all along and had to be asked for. **D131 added the fourth round and a third account to ask it with**: one publish, watched at the same moment by a contact and by a stranger. Neither service announces it to the stranger — and **both hand it to her when she asks outright.** XEP-0163 section 5 makes the `presence` access model a service’s mandatory default, and under it (XEP-0060 section 4.5) retrieval wants a subscription of `from` or `both`; she has neither. It could not be told from here whether the node was created open long ago or the model is never consulted, because neither service will answer a configuration query for its own PEP node. Pinned as the measured fact it is, so that a service becoming stricter turns the round red rather than losing the finding |
| XEP-0045 | Multi-User Chat | ⚠️ | The visiting half: entering a room, being in it, following who is in it, leaving — and the one step of the owner protocol without which a room a join created stays locked for everybody else. Checked against the room services of **both** Prosody and ejabberd, thirteen rounds each, over what a single client cannot do at all: a room that did not exist a moment ago, a second person seen from the first, a nickname refused as taken. **Moderating and inviting came in D117**, ten further rounds against each service: a kick names a nickname and a ban a real address — a role lasts for the visit, an affiliation outlives it — and the status codes that tell a departure from a kick now arrive from a service instead of from a fixture this project wrote itself. **D125 added the one piece of the owner protocol that decides whether a room can be encrypted in** (`muc#roomconfig_whois`, section 10.2.1), and the whole form is fetched and sent back rather than patched — a submit carrying only the changed field tells a service every other setting is now unset, and it answers `result` while quietly resetting the password and the member list. **D130 closed the rest of it**: taking a room down (section 10.9), which is not arrangeable from one side at all — a destruction exists only as something a service does to everybody present, and it is not a departure, though it arrives looking exactly like one, with the address of the room everybody was meant to move to as the only difference; and reading back who is on a room’s lists (section 9.5), which is the only way to know an affiliation took, since a `result` says the service accepted the request and nothing about who ended up on any list. **D132 asked what a room’s archive is worth to somebody with no standing in it**, and the answer is not the one to expect: XEP-0313 puts the test at the right to *enter* at the time of the query, so an open room’s archive is readable by anybody who is not an outcast — and both services are right to hand it over. What that makes worth checking is the other half, which no single query can show: the same archive, the same asker, the same query, after a ban. Both shut it. **D133 closed all three of the leftovers**: a password-protected room asked from both sides of the door — the client half had existed since D116 and had never met a room that wanted one; a voice request (section 8.6), which travels as an ordinary message with no body and no status code, so nothing showed it and the person asking waited while every moderator was told and none of them knew it; and holding a nickname (section 7.10), which is XEP-0077 pointed at a room and was therefore being looked for under the wrong namespace all along. Two more places where the services differ: ejabberd names the nickname it holds and Prosody answers `<registered/>` without one — both are section 7.10, so the reading keeps *is there one* and *what is it* apart — and ejabberd’s default room makes a joiner a participant where Prosody’s makes them a visitor. **D134 did the private message** (section 7.5), which needed three occupants and could therefore not have been written before D131: privacy is a statement about who did *not* get something, and with two accounts there is nobody left over to be that person. Asked twice, because delivery is not the only way out — the second round asks whether the service wrote it into the room’s archive, which D132 showed is open to anybody who may enter. Neither service does either wrong. What it found is on this side: a private message is an ordinary `chat` from `room@service/nick`, so the **last-sent table had every occupant of one room under a single key** — and a correction after two private messages carried the id of the one sent to somebody else, to a person who had never seen what it claimed to replace. The table had been right for eighteen entries because nothing had ever been addressed to an occupant. Still not here: asking the service to invent a room name (section 10.1.4). The first run found that Prosody attaches the `<stanza-id/>` a reply in a room must point at only when the room archives — so a room without an archive is one in which nothing can be answered, which is what `ReplyableId` returning null has meant since D114. **D129 asked ejabberd the same question and got the other answer**: it names a message whether the room archives or not, so the sentence above is Prosody’s behaviour and not a rule. Which is why what is checked there is the client’s half — that a reply points at the room’s name or at nothing, never at the sender’s own id — and each service pins it on the side where it is the strict one. **D129 also found who may invite**: every round until then had the *owner* do the inviting, the one person for whom it cannot fail, and ejabberd’s default room refuses everybody else (`muc#roomconfig_allowinvites` at 0) while Prosody’s allows them. The refusal came back as an ordinary message error that nothing could tell apart from any other, while `InviteToRoomAsync` had already answered true |
...
# The verdict: with Prosody, ejabberd and the oracles up. 131 of 131
dotnet test XMPPConformanceTests/XMPPConformanceTests.csproj
```
